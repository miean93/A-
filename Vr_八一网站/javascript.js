...
(function($, undefined) {
	window.touchSlider = function(options) {
		...
		var ret = {
			...
			prenext = $('.'+ namespace +'-nav a',container);
		...
		if (scroller.css("position") !== "absolute") {
			...
			{				
				viewportHeight = itemheight ? itemheight * $(slides[0]).find('li').length : viewport.height();
			}
        ...
        var loadImage = function(cNode){
            // 更新当前播放进度
            var index = slides.index(cNode)+1;
            var totail = options.slidergroup ?  options.slidergroup : slides.length;
            cNode.find('.position').text(parseInt(index%totail || totail )+'/'+totail);
		...
		$(window).resize(function(){
			...
			prenext.css({top:(viewportHeight-90)/2});
		});
...
$(document).ready(function(){
    $(".pv-slider .slider-pagenation").hover(function(){
       $(this).stop(true,true).animate({right:"0px"},700); 
    },function(){
       $(this).stop(true,true).animate({right:"-200px"},700); 
    });
});
//panel
$(document).ready(function(){
 ...
 $(this).animate({top:'5px'}).animate({top:'-5px'}).animate({top:'0px'});
 $(this).find('span').toggle();
});
 });
/*改革强军_xs*/
 $(document).ready(function(){
    $('.square-xs-nav li').click(function() {
    var $tab_line = $('.square-xs-nav li');
    var index = $tab_line.index(this);
     $(".square-xs-cont").show(200);
    $(".square-xs-cont > div").eq(index).fadeIn().siblings().fadeOut();
    });  
    $(".square-xs-cont-box > span").click(function(){
        $(".square-xs-cont").hide();
        $(".square-xs-cont > div").hide();
    })
    //five-piece
   $(".five-piece .box").each(function(){
    var th = $(this),
        timer = null;
    th.hover(function(){
        timer = setTimeout(function(){
            th.find(".cover").animate({width:'100%',height:'100%'},function(){th.addClass("boxHover").siblings().removeClass("boxHover");});
        }, 300);
    },function(){
        clearTimeout(timer);
        th.removeClass("boxHover");
        th.find(".cover").animate({width:100,height:60});
            });
        });
		 /*CMVIDEO*/
...
+function(){
	...
	function createHTML5video(videoUrl,height){
		return '<video id="video-player" class="video-player" muted=true  preload="none" loop autoplay width="100%" height="'+height+'" src="'+videoUrl+'"></video>';;
	}
	...
	function response(){
		window.onresize = function(){
			var h = $(window).width()*0.562;
			$('#video-player').height(h);
			$('#video-player-obj').height(h);
			$('#container-video').height(h);
			$('.video-image').height(h);
			
		}	
	}

	function fadeInVideoBox(){
	...
	var mediaUrl = $('#video-box').attr('data-url').trim();
	var videoHeight = $(window).width()*0.562;
    var issupporthtml5 = IsSupportHTML5();
	
	if(issupporthtml5 && !isMobile){
		$('#video-box').html(createHTML5video(mediaUrl,videoHeight));
		setTimeout(fadeInVideoBox,100);
	}
...
//视频更多
 $(".video-more").click(function(){
    $(".video-md-list").fadeIn();
  });   
 $(".video-close").click(function(){
    $(".video-md-list").fadeOut();
  }); 
});
/*战味更浓*/
(function ($) {
  $('.square .item').each(function () {
      var self = $(this);
      var el_next = self.next();
      el_next.on('mouseleave', function () {
          el_next.find('div').hide();
          el_next.animate({
              'width': '245px'
          }, 80, function () {
              el_next.hide();
      ...
      self.find('span').on('mouseover', function () {
          el_next.show().animate({
              'width': '855px'
          }, 80, function () {
              el_next.find('div').fadeIn(80);
          });
      });
  });
})(jQuery);